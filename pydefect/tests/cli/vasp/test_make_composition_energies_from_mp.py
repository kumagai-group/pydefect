...
@pytest.fixture
def mp_query(mocker):
    mock = mocker.patch("pydefect.cli.vasp.make_composition_energies_from_mp.MpQuery")
...
def comp_energies():
    return CompositionEnergies({
        'O8': CompositionEnergy(-39.58364375, "mp-1"),
        'Mg3': CompositionEnergy(-4.79068775, "mp-2"),
        'Mg1O1': CompositionEnergy(-11.96742144, "mp-3")})
...
def comp_energies_corr():
    return CompositionEnergies({
        'O8': CompositionEnergy(-39.58364375 + diff["O"] * 8, "mp-1"),
        'Mg3': CompositionEnergy(-4.79068775 + diff["Mg"] * 3, "mp-2"),
        'Mg1O1': CompositionEnergy(-11.96742144 + diff["Mg"] + diff["O"], "mp-3")})


def test_make_chem_pot_diag_from_mp(mp_query, comp_energies):
...
def test_make_chem_pot_diag_from_mp_yaml(mp_query, tmpdir, comp_energies_corr):
    tmpdir.chdir()
...
def test_remove_higher_energy_comp():
    comp_es = CompositionEnergies({'Mg1': CompositionEnergy(-1.9, "mp-2"),
                                   'Mg2': CompositionEnergy(-4.0, "mp-3"),
                                   'O2': CompositionEnergy(-4.0, "mp-4"),
                                   })
    actual = remove_higher_energy_comp(comp_es)
    expected = CompositionEnergies({'Mg2': CompositionEnergy(-4.0, "mp-3"),
                                    'O2': CompositionEnergy(-4.0, "mp-4"),})
