...
def test_make_defect_energy_summary(mocker):
    ...
    defect_infos = [DefectEnergyInfo("Va_Mg1", 0, {Element.Mg: -1}, de1),
                    DefectEnergyInfo("Va_Mg1", 1, {Element.Mg: -1}, de2)]
    rel_chem_pot = {"A": {Element.Mg: 5.0}}

    unitcell = mocker.Mock()
    perf_be_state = mocker.Mock()

    actual = make_defect_energy_summary(defect_infos, rel_chem_pot, unitcell, perf_be_state)
